...
bool    BitcoinExchange::_parseInput(std::ifstream &input)
{
    ...
    if (!std::getline(input, line))
        return (std::cerr << EMPTY << std::endl, 1);
    line.erase(std::remove_if(line.begin(), line.end(), ::isspace), line.end());
    for (std::string::size_type i = 0; i < line.size(); i++)
        line[i] = std::tolower(line[i]);
    if ((found = line.find("|")) == std::string::npos)
        return (std::cerr << FORMAT << std::endl, 1);
...
bool BitcoinExchange::_findClosestDate(const std::string &inputDate) const
{
    ...
    std::cout << closestDate << " => " << closestValue << std::endl;
    return (0);
}